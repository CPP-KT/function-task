...
struct tracking_destructor_func {
  tracking_destructor_func(size_t* destructor_called) noexcept : destructor_called(destructor_called) {}

  void operator()() const noexcept {}

  ~tracking_destructor_func() {
    ++*destructor_called;
  }

  size_t* destructor_called;
};

struct large_tracking_destructor_func : tracking_destructor_func {
  using tracking_destructor_func::tracking_destructor_func;

  [[maybe_unused]] int payload[1000];
};

TEST(function_test, destructor_called_in_move_assignment_to_small) {
  size_t d1 = 0;
  size_t d2 = 0;

  {
    function<void()> large = large_tracking_destructor_func(&d1);
    EXPECT_FALSE(is_small<large_tracking_destructor_func>(large));
    EXPECT_EQ(1, d1);

    function<void()> small = tracking_destructor_func(&d2);
    EXPECT_TRUE(is_small<tracking_destructor_func>(small));
    EXPECT_EQ(1, d2);
...
TEST(function_test, destructor_called_in_move_assignment_to_large) {
  size_t d1 = 0;
  size_t d2 = 0;

  {
    function<void()> small = tracking_destructor_func(&d1);
    EXPECT_TRUE(is_small<tracking_destructor_func>(small));
    EXPECT_EQ(1, d1);

    function<void()> large = large_tracking_destructor_func(&d2);
    EXPECT_FALSE(is_small<large_tracking_destructor_func>(large));
    EXPECT_EQ(1, d2);
